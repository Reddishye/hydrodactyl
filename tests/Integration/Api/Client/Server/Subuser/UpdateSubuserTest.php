...
class UpdateSubuserTest extends ClientApiIntegrationTestCase
{
    ...
    public function testCorrectPermissionsAreRequiredForUpdating()
    {
        ...
        $this->postJson(
            $endpoint = "/api/client/servers/wings/$server->uuid/users/{$subuser->user->uuid}",
            $data = [
                'permissions' => [
                    'control.start',
                    'control.stop',
    ...
    public function testPermissionsAreSavedToAccount()
    {
        ...
        $this->actingAs($user)
            ->postJson("/api/client/servers/wings/$server->uuid/users/{$subuser->user->uuid}", [
                'permissions' => [
                    'control.start',
                    'control.stop',
                    'control.stop',
    ...
    public function testUserCannotUpdateSelf()
    {
        ...
        $this->actingAs($user)
            ->postJson("/api/client/servers/wings/$server->uuid/users/$user->uuid", [])
            ->assertForbidden();
    }
    ...
    public function testCannotUpdateSubuserForDifferentServer()
    {
        ...
        $this->actingAs($user)
            ->postJson("/api/client/servers/wings/$server->uuid/users/$user2->uuid", [])
            ->assertNotFound();
    }
}
