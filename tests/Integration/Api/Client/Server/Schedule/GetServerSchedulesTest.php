...
class GetServerSchedulesTest extends ClientApiIntegrationTestCase
{
    ...
    public function testServerSchedulesAreReturned(array $permissions, bool $individual)
    {
        ...
        $response = $this->actingAs($user)
            ->getJson(
                $individual
                    ? "/api/client/servers/wings/$server->uuid/schedules/$schedule->id"
                    : "/api/client/servers/wings/$server->uuid/schedules"
            )
            ->assertOk();
    ...
    public function testScheduleBelongingToAnotherServerCannotBeViewed()
    {
        ...
        $this->actingAs($user)
            ->getJson("/api/client/servers/wings/$server->uuid/schedules/$schedule->id")
            ->assertNotFound();
    }
    ...
    public function testUserWithoutPermissionCannotViewSchedules()
    {
        ...
        $this->actingAs($user)
            ->getJson("/api/client/servers/wings/$server->uuid/schedules")
            ->assertForbidden();
